...
const statusConfig = {
  ...
  running: {
    icon: Scan,
    color: "text-[var(--ghost-accent-primary)]",
    bg: "bg-[var(--ghost-accent-primary)]/10",
    label: "Running",
  },
  pending: {
    icon: Clock,
    color: "text-[var(--ghost-text-muted)]",
    bg: "bg-[var(--ghost-text-muted)]/10",
    label: "Pending",
  },
}
  ...
  return (
    <div className="space-y-6">
      ...
      <GhostCard className="p-6">
        {tests.length === 0 ? (
          <p className="text-sm text-[var(--ghost-text-subtle)] text-center py-8">
          ...
          <div className="space-y-4">
            {tests.map((test) => {
              ...
              return (
                <div
                  key={test.id}
                  className="flex items-center justify-between border-b border-[var(--ghost-border)] pb-4 last:border-b-0 last:pb-0"
                >
                    ...
                    <div>
                      <p className="text-white">
                        ...
                        {test.overall_score !== null && test.status === "completed" && (
                          <span className="text-[var(--ghost-accent-primary)] ml-2 font-mono tabular-nums">
